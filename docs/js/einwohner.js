/* einwohner.js – Durchsuchbares Einwohnerverzeichnis */
...
(function () {
  ...
  function preprocess(data) {
    for (const d of data) {
      d._search = [d.nachname, d.vorname, d.beruf, d.adresse]
  ...
  function render() {
    ...
    const rows = slice.map(d => `<tr>
      <td>${escHtml(d.nachname)}</td>
      <td>${escHtml(d.vorname)}</td>
      <td>${escHtml(d.beruf)}</td>
